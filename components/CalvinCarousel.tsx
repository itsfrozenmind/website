...
export default function CalvinCarousel({ strips }: { strips: Strip[] }) {
  ...
  if (strips.length === 0) {
    return (
      <div className="rounded-sm p-6 text-center" style={{ background: 'var(--surface-2)', border: '1px solid var(--border)' }}>
  ...
  return (
    <div>
      <div className="rounded-sm overflow-hidden mb-3" style={{ background: 'var(--surface-2)', border: '1px solid var(--border)' }}>
      ...
      <div className="flex items-center justify-between">
        <div className="flex gap-1 items-center">
          <button onClick={() => setCurrent(c => (c - 1 + strips.length) % strips.length)}
            className="mono px-2.5 py-1.5 rounded-sm transition-colors hover:border-[var(--accent)]"
            style={{ border: '1px solid var(--border)', color: 'var(--muted)', fontSize: '0.65rem' }}>←</button>
        ...
        <button onClick={handleShare}
          className="mono px-3 py-1.5 rounded-sm transition-colors hover:border-[var(--accent)]"
          style={{ border: '1px solid var(--border)', color: copied ? 'var(--green)' : 'var(--muted)', fontSize: '0.65rem' }}>
